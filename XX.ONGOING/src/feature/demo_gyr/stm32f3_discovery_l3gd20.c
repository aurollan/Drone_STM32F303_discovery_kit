...
void L3GD20_Init(L3GD20_InitTypeDef *L3GD20_InitStruct) {
  ...
  ctrl1 |=
      (uint8_t)(L3GD20_InitStruct->Power_Mode |
                L3GD20_InitStruct->Output_DataRate |
                L3GD20_InitStruct->Axes_Enable | L3GD20_InitStruct->Band_Width);

  ctrl4 |=
      (uint8_t)(L3GD20_InitStruct->BlockData_Update |
                L3GD20_InitStruct->Endianness | L3GD20_InitStruct->Full_Scale);
 ...
 * @param  L3GD20_InterruptConfig_TypeDef: pointer to a
 * L3GD20_InterruptConfig_TypeDef structure that contains the configuration
 * setting for the L3GD20 Interrupt.
 * @retval None
 */
void L3GD20_INT1InterruptConfig(
    L3GD20_InterruptConfigTypeDef *L3GD20_IntConfigStruct) {
 ...
 * structure that contains the configuration setting for the L3GD20.
 * @retval None
 */
 ...
 * @retval None
 */
void L3GD20_Write(uint8_t *pBuffer, uint8_t WriteAddr,
                  uint16_t NumByteToWrite) {
  /* Configure the MS bit:
       - When 0, the address will remain unchanged in multiple read/write
     commands.
...
static void L3GD20_LowLevel_Init(void) {
  ...
  RCC_AHBPeriphClockCmd(L3GD20_SPI_SCK_GPIO_CLK | L3GD20_SPI_MISO_GPIO_CLK |
  ...
  GPIO_PinAFConfig(L3GD20_SPI_SCK_GPIO_PORT, L3GD20_SPI_SCK_SOURCE,
                   L3GD20_SPI_SCK_AF);
  GPIO_PinAFConfig(L3GD20_SPI_MISO_GPIO_PORT, L3GD20_SPI_MISO_SOURCE,
                   L3GD20_SPI_MISO_AF);
  GPIO_PinAFConfig(L3GD20_SPI_MOSI_GPIO_PORT, L3GD20_SPI_MOSI_SOURCE,
                   L3GD20_SPI_MOSI_AF);
